...
window.Vaadin.Flow['_vaadintheme_demovaadin_globalCss'] = window.Vaadin.Flow['_vaadintheme_demovaadin_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_demovaadin_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_demovaadin_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_demovaadin_globalCss'].push(target);
  }
  if (!document['_vaadintheme_demovaadin_componentCss']) {
    
    document['_vaadintheme_demovaadin_componentCss'] = true;
  }
